...
def test_reannotate_fails(create_testfolder, database_paths_kiarva):
    ...
    with pytest.raises(KeyError):
        ddl.pp.reannotate_genes(
            create_testfolder,
            igblast_db=database_paths_kiarva["igblast_db"],
...
@pytest.mark.usefixtures("database_paths_kiarva")
def test_updateblastdb(database_paths_kiarva):
    """test_updateblastdb"""
    makeblastdb(database_paths_kiarva["blastdb_fasta"])
...
def test_assignsisotypes(
    ...
    ddl.pp.assign_isotypes(
        create_testfolder,
        blastdb=database_paths_kiarva["blastdb_fasta"],
        filename_prefix=filename,
        save_plot=True,
        show_plot=False,
    )
...
@pytest.mark.usefixtures(
    "create_testfolder", "processed_files", "database_paths_kiarva"
)
def test_create_germlines(
    ...
    f = create_testfolder / "dandelion" / processed_files["filtered"]
    ddl.pp.create_germlines(f, germline=database_paths_kiarva["germline"])
...
def test_store_germline_references(
    ...
    f = create_testfolder / "dandelion" / processed_files["filtered"]
    vdj = ddl.Dandelion(f)
    vdj.store_germline_reference(germline=database_paths_kiarva["germline"])
