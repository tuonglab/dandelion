    ...
    if obsp:
        ...
        if not skip_stash:
            ...
            recipient.uns.setdefault("dandelion", {})[g_connectivities_key] = (
                recipient.obsp["connectivities"].copy()
            )
    ...
    if obsm:
        ...
        if vdj.layout is not None:
            ...
            if active_obsm is not None:
                _ddl = recipient.uns.setdefault("dandelion", {})
                recipient.obsm["X_vdj"] = _ddl[active_obsm].copy()
...
def clone_view(
    ...
    if mode is None:
        ...
        if connectivities_key in _ddl:
            adata.obsp["connectivities"] = _ddl[connectivities_key].copy()
        else:
            raise KeyError(
        ...
        if distances_key in _ddl:
            adata.obsp["distances"] = _ddl[distances_key].copy()
        else:
            raise KeyError(
        ...
        adata.obsp["connectivities"] = _ddl[conn_key].copy()
        adata.obsp["distances"] = _ddl[dist_key].copy()
        if emb_key is not None:
            if emb_key not in _ddl:
                raise KeyError(f"{emb_key} not found in adata.uns['dandelion']")
            adata.obsm["X_vdj"] = _ddl[emb_key].copy()
...
def clone_overlap(
    ...
    datc_[group_by] = [dictg_[l] for l in datc_["cell_id"]]
...
def vj_usage_pca(
    ...
    **kwargs
    ...
    if transfer_mapping is not None:
        collapsed = adata_.obs.drop_duplicates(subset=group_by)
