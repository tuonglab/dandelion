...
def find_clones(
    ...
    # Initialize Zarr store for streaming distance writes if requested
    zarr_writer = None
    _zarr_path = None
    _embed_pending = False
    if store_distances and lazy:
        # Use provided path or create temp directory
        if zarr_path is None:
            _zarr_path = tempfile.mkdtemp()
            _embed_pending = True  # Will embed in zipddl when saved
        else:
            _zarr_path = str(zarr_path)
            _embed_pending = False  # External path, don't embed
        zarr_writer = _ZarrDistanceWriter(
                ...
                if seqs_non_empty:
                    ...
                    if len(meta_indices) > 0 and d_mat_unique.size > 0:
                        ...
                        unique_indices = np.array(
                            [seq_to_unique_idx[seq] for seq in seqs_non_empty]
                        )
                        d_mat = d_mat_unique[
                            np.ix_(unique_indices, unique_indices)
                        ]
                        if store_distances:
                            # Store distances - either to Zarr or in memory
                            if zarr_writer is not None:
                                # Write directly to Zarr array
                                zarr_writer.write_submatrix(meta_indices, d_mat)
                            else:
...
def group_pairwise_hamming_distance(
    ...
    for g in tqdm(
        ...
        for length in clonotype_sequence_group[g]:
            seq_ = list(clonotype_sequence_group[g][length])
            ...
            tr = math.floor(int(length) * (1 - identity))
            ...
            for x in range(0, len(clones_tmp)):
                clones[g][length][x + 1] = clones_tmp[x]
    # now to retrieve the contig ids that are grouped together
    cid = Tree()
    for g in clones:
        for length in clones[g]:
            # retrieve the clone 'numbers'
            for c in clones[g][length]:
                grp_seq = clones[g][length][c]
                for key, value in clonotype_vj_len_group[g][length].items():
                    if value in grp_seq:
                        cid[g][length][c][key].value = 1
...
def rename_clonotype_ids(
    ...
    for g in clonotype_groups:
        ...
        for length in clonotype_groups[g]:
            third_key = []
            for k3 in clonotype_groups[g][length].keys():
                third_key.append(k3)
            third_key = list(set(third_key))
            third_key_dict = dict(zip(third_key, range(1, len(third_key) + 1)))
            for key, value in dict(clonotype_groups[g][length]).items():
                for v in value:
                    if type(v) is int:
                        break
                    clone_dict[v] = (
